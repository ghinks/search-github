...
const createQuery = (owner, name, cursor, states = 'OPEN') => {
  ...
  query {
    repository(owner: "${owner}", name: "${name}") {
      id,
      url,
      issues(first: ${_constants2.default.pageSz}, states: [${states}] , orderBy: { field: CREATED_AT, direction: DESC } ${after}) {
        totalCount,
        nodes {
          author {
            login
...
const getIssues = (() => {
  var _ref3 = (0, _asyncToGenerator3.default)(function* (owner, name, searchTerms, states) {
  ...
  return function getIssues(_x9, _x10, _x11, _x12) {
